...
type UpdateUser struct {
	...
	Telephone string `json:"telephone" validate:"omitempty,min=1,max=50"`

	//员工工号，长度最大50个字符
	JobNumber string `json:"job_number" validate:"omitempty,min=1,max=50"`
	...
	OrgEmail string `json:"org_email" validate:"omitempty,max=100"`

	//员工的企业邮箱类型，仅对支持的组织生效（profession: 标准版，base: 基础版）
	OrgEmailType string `json:"org_email_type" validate:"omitempty,oneof=profession base"`

	//办公地点，长度最大100个字符
	WorkPlace string `json:"work_place" validate:"omitempty,max=100"`
	...
	//扩展属性，长度最大2000个字符
	Extension string `json:"extension" validate:"omitempty,max=2000"`
...
func (ub *updateUserBuilder) SetLanguage(language string) *updateUserBuilder {
